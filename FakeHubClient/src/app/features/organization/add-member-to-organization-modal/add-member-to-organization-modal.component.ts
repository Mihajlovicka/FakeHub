...
@Component({
  selector: "app-add-member-to-organization-modal",
  standalone: true,
  imports: [
    MatDialogContent,
    MatDialogActions,
  ...
  styleUrl: "./add-member-to-organization-modal.component.css",
})
export class AddMemberToOrganizationModalComponent
  implements OnInit, OnDestroy
{
  private readonly dialogRef = inject(
    MatDialogRef<AddMemberToOrganizationModalComponent>
  );
  private userService: UserService = inject(UserService);
  public readonly organizations = inject(MAT_DIALOG_DATA).organizations;
  public readonly dialogUsers$: BehaviorSubject<UserProfileResponseDto[]> = inject(MAT_DIALOG_DATA).users;
