...
export class ViewOrganizationsMembersComponent implements OnInit, OnChanges {
  ...
  public search(): void {
    ...
    for (const q of queriesUsername) {
      const userNameExact = this.users.filter((u) => u.username.toLowerCase() === q);
      const userNameContains = this.users.filter((u) => u.username.toLowerCase().includes(q));
      result = Array.from(new Set([...result, ...userNameExact, ...userNameContains]));
    }

    for (const q of queriesEmail) {
      const emailExact = this.users.filter((u) => u.email.toLowerCase() === q);
      const emailContains = this.users.filter((u) => u.email.toLowerCase().includes(q));
      result = Array.from(new Set([...result, ...emailExact, ...emailContains]));
    }

    this.filteredUsers = result.sort((a, b) => {
      const isExactUsernameA = queriesUsername.includes(a.username.toLowerCase());
      const isExactUsernameB = queriesUsername.includes(b.username.toLowerCase());
      ...
      const isPartialUsernameA = queriesUsername.some((q) => a.username.toLowerCase().includes(q));
      const isPartialUsernameB = queriesUsername.some((q) => b.username.toLowerCase().includes(q));

      const isPartialEmailA = queriesEmail.some((q) => a.email.toLowerCase().includes(q));
      const isPartialEmailB = queriesEmail.some((q) => b.email.toLowerCase().includes(q));

      if (isExactUsernameA !== isExactUsernameB) return isExactUsernameA ? -1 : 1;
      if (isExactEmailA !== isExactEmailB) return isExactEmailA ? -1 : 1;
      if (isPartialUsernameA !== isPartialUsernameB) return isPartialUsernameA ? -1 : 1;
  ...
  public openDeleteMemberModal(user: UserProfileResponseDto): void {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      data: {
        title: "Delete member",
        description: "Would you like to delete \"" + user.username + "\" from organization?"
      },
    });
    dialogRef.afterClosed().subscribe((isConfirmed) => {
      if (isConfirmed) {
        this.service.deleteMember(this.organizationName, user.username).subscribe(
          data => {
            if(data?.username != null) {
              const userIndex = this.filteredUsers.findIndex(u => u.username == data.username);
